...
public class InSharesEquityTradeExtractorTests
{
    [Fact]
    public void Extract_WellFormedEmailBody_ShouldReturnExtractedEquityTrade()
    {
        ...
        var tradeData = "Your market order to buy " +
            $"1.25 shares of ABC was executed " +
            $"at an average price of $1,234.56 " +
            $"on March 21st, 2021 at 2:30 PM.";

        var email = new EmailInputDomainModel { Body = $"abcdefg <div>{tradeData}</div> abcdefg" };

        var extractor = new InSharesEquityTradeExtractor();
